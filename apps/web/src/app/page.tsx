...
const features = [
  ...
  },
  {
    ...
    iconClass: 'bg-medical-sky/15 text-medical-sky',
  },
  {
    icon: ShieldCheck,
...
const personas = [
  {
    ...
    accent: 'from-primary/10 to-medical-sky/10',
  },
  {
    icon: HeartPulse,
    title: 'For patients',
...
export default function LandingPage() {
  return (
    <div className="medical-gradient-bg text-foreground min-h-screen">
      <header className="border-border/80 bg-card/70 sticky top-0 z-50 border-b backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
          <Link href="/" className="flex items-center gap-2.5 font-semibold">
            <Image
              ...
              width={36}
              height={36}
              className="ring-primary/20 size-9 rounded-full ring-2"
              priority
            />
            <span className="text-primary">Sunrise Medical</span>
          </Link>
      ...
      <main>
        ...
        >
          <div
            aria-hidden
            className="absolute inset-0 bg-linear-to-br from-primary/15 via-background/70 to-sunrise/10"
          />
          <div className="relative mx-auto grid max-w-6xl items-center gap-12 px-6 py-24 md:grid-cols-2">
            <div className="space-y-6">
              <span className="bg-primary/12 text-primary border-primary/20 inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium">
                AI-powered healthcare CRM
              </span>
              <h1 className="text-4xl font-bold tracking-tight md:text-5xl">
                <span className="text-primary">Modern care,</span> organized.
              </h1>
          ...
          <div className="mx-auto max-w-6xl px-6 py-20">
            <div className="mx-auto max-w-2xl text-center">
              <h2 className="text-3xl font-bold tracking-tight">
                Everything your <span className="text-primary">clinic</span> needs
              </h2>
            ...
            <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {features.map((feature) => {
                ...
                return (
                  <div
                    key={feature.title}
                    className="medical-card-glow bg-card hover:border-primary/30 rounded-xl border p-6 transition-colors"
                  >
                    <span
                      className={`mb-4 inline-flex h-10 w-10 items-center justify-center rounded-lg ${feature.iconClass}`}
                    >
          ...
          <div className="mx-auto grid max-w-6xl gap-6 px-6 py-20 md:grid-cols-2">
            {personas.map((persona) => {
              ...
              return (
                <div
                  key={persona.title}
                  className={`medical-card-glow bg-linear-to-br ${persona.accent} bg-card flex flex-col rounded-2xl border p-8`}
                >
                  <span className="bg-primary/15 text-primary mb-4 inline-flex h-12 w-12 items-center justify-center rounded-xl">
                  ...
                  <Button asChild className="mt-6 w-fit shadow-sm">
      ...
      <footer className="border-t bg-card/50">
        <div className="text-muted-foreground mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-6 py-6 text-sm sm:flex-row">
          <p>© {new Date().getFullYear()} Sunrise Medical. All rights reserved.</p>

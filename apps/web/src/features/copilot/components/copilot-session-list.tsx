...
import type { ChatSession } from '@/features/copilot/types/copilot.type';
        ...
        {isLoading ? (
          ...
          <ul className="space-y-2">
            {sessions.map((session) => {
              const preview =
                session.messages?.[0]?.content ?? 'Ask about patients or clinic activity';
                ...
                <li key={session.id}>
                  <button
                    ...
                    className={cn(
                      ...
                      activeId === session.id && 'border-primary bg-primary/5',
                    )}
                  >
